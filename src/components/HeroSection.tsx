...
export function HeroSection() {
  ...
  useEffect(() => {
    // Fallback timer: start animations after 1.8s in case the video is slow or autoplay fails
  ...
  return (
    <section id="home" className="relative flex min-h-screen items-center overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 pt-28">
        <p className={`${videoPlaying ? "animate-fade-rise" : "opacity-0"} text-[0.7rem] uppercase tracking-[0.42em] text-gold`}>
          Study &nbsp;/&nbsp; Work &nbsp;/&nbsp; Migrate
        </p>

        <h1 className="mt-6 max-w-4xl font-display text-5xl leading-[1.05] text-primary-foreground sm:text-7xl lg:text-8xl">
            ...
            <span key={wi} className="mr-[0.28em] inline-block whitespace-nowrap">
              {word.split("").map((ch, ci) => (
                <span
                  ...
                  style={{ animationDelay: `${0.25 + wi * 0.22 + ci * 0.045}s` }}
                >
                  {ch}
                </span>
              ))}
        ...
        <div className={`${videoPlaying ? "animate-line-grow" : "scale-x-0"} mt-8 h-px w-56`} style={{ background: "var(--gradient-gold)" }} />
        ...
        <div className={`${videoPlaying ? "animate-fade-rise" : "opacity-0"} mt-10 flex flex-wrap gap-4 pb-24`} style={{ animationDelay: "1.9s" }}>
          <a
            href="#contact"
